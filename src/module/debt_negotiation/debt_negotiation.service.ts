...
export class DebtNegotiationService {
  ...
  async dividasNovaVersao(codigo_matricula: number, preinscricaoId: number): Promise<any[]> {
    ...
    for (const aa of anosAnterioresRaw) {
      ...
      const mesesIds = await this.mesesPagosPorAnoPropina(ano.ano_lectivo, codigo_inscricao);
      console.log(mesesIds, "ISAAC - TESTE");



      const mesesIsentosIds: any = await this.getPrestacoesAnosAnterioresPorAnoLectivo(ano.ano_lectivo);


      console.log(mesesIds, mesesIsentosIds, "MESES PAGOS E ISENTOS", mesesIsentosIds);
...
INNER JOIN "UMA_PROPINA_POR_CURSO" ppc 
  ON ppc."codigo_servico" = ts."Codigo"
INNER JOIN "UMA_MESES" m 
  ON m."codigo" = ppc."mes_id"
INNER JOIN "UMA_TB_ANO_LECTIVO" al 
...
`;
      ...
      if (mesesIds.length > 0) {
        filtros += ` AND ppc."mes_id" NOT IN (${mesesIds.map((_, i) => `:${params.length + i + 1}`).join(',')})`;
        params.push(...mesesIds);
      }

      if (mesesIsentosIds.length > 0) {
        filtros += ` AND ppc."mes_id" NOT IN (${mesesIsentosIds.map((_, i) => `:${params.length + i + 1}`).join(',')})`;
        params.push(...mesesIsentosIds);
      }

      const mesesNaoPagos = await this.dataSource.query(query + filtros, params);
  ...
  async getDebt(enrrolmentId: number, codigo_inscricao: number, tipo: number) {
    ...
    const mesesDividas = dividas
      ...
      .sort((a, b) => a.ano_lectivo?.localeCompare(b.ano_lectivo));
